 ...
 *
 * @param apiKey  Bearer-токен из openrouter.ai
 * @param baseUrl Базовый URL (по умолчанию OpenRouter)
 * @param model   Идентификатор модели (например "openai/gpt-4o-mini")
 */
class OpenRouterClient(
     ...
     */
    fun chat(userMessage: String, systemPrompt: String? = null): String {
        ...
        if (!response.isSuccessful) {
            throw RuntimeException("API ошибка ${response.code}: $responseBody")
        }
    ...
    /**
     * Анализирует результаты проверки задания и возвращает структурированный ревью.
     *
     * @param checkResults  Результаты автоматических чекеров (текст)
     * @param candidateName Имя кандидата
     * @param finalScore    Итоговый балл (0..100 или null)
     * @return Текст ревью в markdown-формате
     */
    fun reviewSubmission(
        ...
        val systemPrompt = """
            Ты — опытный ревьюер кода. Проанализируй результаты автоматической проверки и дай заключение.
